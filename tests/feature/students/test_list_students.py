...
def test_a_user_can_view_a_list_of_their_students(test_client, test_db):
...
def test_guest_can_not_view_list_of_students(test_client, test_db):
    user = add_user("Test User", "user@test.com", "test1234")
    add_student(user, "Test Student", "student@test.com", "2009FS-ON", "UCFD")

    response = test_client.get("/students", follow_redirects=True)
    result = response.data.decode()
